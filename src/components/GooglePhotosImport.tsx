...
export function GooglePhotosImport({ onImportComplete }: Props) {
  ...
  const startPicker = async () => {
    ...
    setStage("creating");
    try {
      const res = await fetch("/api/google-photos/picker/session", { method: "POST", credentials: "include" });
      ...
      pollRef.current = setInterval(async () => {
        try {
          const pollRes = await fetch(`/api/google-photos/picker/session/${sid}`, { credentials: "include" });
          ...
          if (data.ready && data.items) {
            ...
            if (popupRef.current && !popupRef.current.closed) popupRef.current.close();
            setItems(data.items.filter(i => i.mediaFile.mimeType.startsWith("image/")));
            setStage("confirming");
          }
        } catch (e) {
          clearInterval(pollRef.current!);
  ...
  if (stage === "done") {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-6">
        <div className="text-5xl font-light text-slate-900">{importedCount}</div>
        <div className="text-slate-500 font-light">photos imported successfully</div>
        <button
          onClick={onImportComplete}
          className="px-6 py-2.5 bg-slate-800 text-white rounded-lg text-sm hover:bg-slate-700 transition-colors"
        >
          View Timeline
        </button>
      </div>
  ...
  if (stage === "confirming") {
    return (
      <div>
        ...
        <div className="mb-6">
          <p className="text-sm font-light text-slate-500 mb-2">Frame</p>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-6">
      <div className="text-slate-700 font-light text-lg">Import from Google Photos</div>
      ...
      {stage === "idle" && (
        <button
          onClick={startPicker}
          className="flex items-center gap-2 px-6 py-3 bg-slate-800 text-white rounded-lg text-sm font-normal hover:bg-slate-700 transition-colors"
        >
          Open Google Photos
        </button>
      )}

      {stage === "creating" && <div className="text-slate-400 text-sm">Opening picker…</div>}
      ...
      {stage === "picking" && (
        <div className="flex flex-col items-center gap-4">
          ...
          <button onClick={cancelPicking} className="text-slate-400 hover:text-slate-600 text-sm transition-colors">
            Cancel
          </button>
        </div>
      )}
